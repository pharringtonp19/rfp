...
def plot_data(batch, predict=None):
    ...
    plt.xlabel(r"$X$")
    fig.savefig(
        file_link + f"motivating_cefs_{len(X)}_{predict[2]}_{predict[3]}.png",
        format="png",
    )
    plt.show()
...
def get_data(key, n_obs):
    ...
    x_start = jax.random.uniform(subkey1, shape=(1,), minval=-3.0, maxval=-2.0)
    lenth = jax.random.uniform(subkey2, shape=(1,), minval=1.0, maxval=4.0)
...
def main(argv) -> None:
    ...
    batch = jax.vmap(get_data, in_axes=(0, None))(
        jax.random.split(jax.random.PRNGKey(2), FLAGS.c), FLAGS.n
    )
    ...
    if FLAGS.full:
        ...
        inner_opt = optax.multi_transform(
            {"train": optax.sgd(learning_rate=0.001), "zero": set_to_zero()},
            Model_Params("zero", "train", "train"),
        )

    inner_yuri = Trainer(loss_fn=supervised_loss, opt=inner_opt, epochs=3)
