...
def main(argv):
    ...
    loss_control = np.load(np_file_link + f"ode1_loss_control_{FLAGS.person}.npy")
    loss_treated = np.load(np_file_link + f"ode1_loss_treated_{FLAGS.person}.npy")
    ...
    filename = file_link + f"1ode_loss_{FLAGS.person}.pdf"
    fig.savefig(filename, format="pdf")
    plt.show()

    xlabel = "Renter" if FLAGS.person else "Occupied Unit"
    est_effect = np.load(np_file_link + f"ode1_est_effect_{FLAGS.person}.npy")
    annualized_effect = 52 * est_effect
    ts = np.load(np_file_link + f"ode1_eviction_rt_{FLAGS.person}.npy")
    ts = ts
    print(np.max(ts))
    fig = plt.figure(dpi=300, tight_layout=True)
    ...
    plt.legend(frameon=False)
    filename = file_link + f"1ode_est_effect_{FLAGS.person}.pdf"
    fig.savefig(filename, format="pdf")
    plt.show()

    es = np.load(np_file_link + f"ode1_multi_run_0.8_{FLAGS.person}.npy")
    annualized_es = es * 52
    ...
    plt.legend(frameon=False)
    filename = file_link + f"1ode_est_effect_sampling_{FLAGS.person}.pdf"
    fig.savefig(filename, format="pdf")
    plt.show()
